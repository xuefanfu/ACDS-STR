    ...
    def __init__(self, root, opt):
        ...
        with self.env.begin(write=False) as txn:
            ...
            if self.opt.data_filtering_off:
                ...
                """
                if not self.opt.is_mask_supervision:
        ...
        with self.env.begin(write=False) as txn:
            ...
            if not self.opt.is_mask_supervision:
                ...
                if self.opt.eval:
                    ...
                    # print("word{},--{}".format(label,label["word"]))
                    label["word"] = re.sub(out_of_char,'', label["word"])
                    weight,height=img.size
                    label = str(label)+"_"+str(weight)+"_"+str(height)
    ...
    def __init__(self, opt):
        ...
        if not opt.eval:
            ...
            if not self.opt.is_mask_supervision:
                ...
                self.camera = [Contrast(), Brightness(), JpegCompression()]
            ...
            if self.opt.isrand_aug:
                if not self.opt.is_mask_supervision:
                    ...
                    self.augs = [self.process, self.camera, self.noise, self.blur, self.weather, self.pattern]
            # semantic augment
...
class AlignCollate(object):
    ...
    def __call__(self, batch):
        ...
        if self.keep_ratio_with_pad:  # same concept with 'Rosetta' paper
            ...
            if self.opt.close_aug:
                image_tensors = torch.cat([(transforms.ToTensor()(t.resize((self.opt.imgW, self.opt.imgH), Image.BICUBIC))).unsqueeze(0) for t in images], 0)
            else:
